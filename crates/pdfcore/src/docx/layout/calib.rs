...
//!
//! 每个开关都要写明现在的取值依据。[`Calib::legacy`] 是重写前引擎的规则：重写期间
//! 新引擎按它排出来的结果与旧引擎逐坐标一致，以此证明「没改的地方真没变」；旧引擎
//! 已在重写完成后删除，这组取值留给 `tests/oracle.rs` 的 `calibrate` 对比每条规则
//! 改动前后与参照的差距。
...
pub enum Images {
    ...
    ///
    /// 浮动的图（`wp:anchor`）在锚点段落的第一行放下时定位，段落挪到下一页就跟过去：
    /// 横向相对纸张、版心、栏、页边，纵向相对纸张、版心、段落顶、行顶，按偏移或对齐；
    /// 衬于文字下方的先画、其余的在正文之后画；上下型环绕让图所在的那一段横条空着，
    /// 碰到的行（包括锚点段落自己的）挪到图下面；四周型、紧密型、穿越型按上下型近似。
    /// 单元格、页眉页脚里的浮动图相对所在的栏与段落。
    ///
    /// 形状与文本框（`wps:wsp`，以及 VML 的 `v:shape`、`v:rect`、`v:line`……）与图片
    /// 同样排：矩形先填充、再描边（线骑在外框上），直线是外框的一条对角线，翻转换成
    /// 另一条；框里的字按框宽减去左右边距（缺省 7.2pt）排，从上边距（缺省 3.6pt）处
    /// 往下，按 `@anchor` 竖直对齐。框里的字不吸附行网格，行尾标点照样悬挂（单元格里
    /// 不悬挂）；放不下时，接着的那一行只要顶端还在文字区里就画、裁到文字区底边，再
    /// 往后的不画。`mc:AlternateContent` 取 `wps` 的那一支。VML 的图（`v:imagedata`）
    /// 当图片画。圆角矩形按矩形画，旋转的按不旋转画，其他几何形状只排框里的字，都汇总
    /// 成一条警告；组合、图表画灰框。
    ///
    /// 与 LibreOffice 不同处：它把 VML 文本框当成自己的文本框架，边框整个画在框里、
    /// 字再往里缩一个线宽；这里按 VML 的规矩，与 DrawingML 的文本框一样量。
    Inline,
}
